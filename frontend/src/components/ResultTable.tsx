...
export function ResultTable({ data }: { data: unknown }) {
  ...
  return (
    <section className="mt-4 overflow-hidden rounded-xl border border-accent/10 bg-white/80 shadow-line">
      ...
      <div className="max-h-[360px] overflow-auto">
        <table className="min-w-full border-separate border-spacing-0 text-left text-sm">
          <thead className="sticky top-0 z-10 bg-[#eaf1fb]">
            <tr>
              {columns.map((column) => (
                <th
                  key={column}
                  scope="col"
                  className="border-b border-slate/10 px-4 py-3 font-semibold text-slate/70"
                >
                  {column}
                </th>
              ))}
          ...
          <tbody>
            {rows.map((row, rowIndex) => (
              <tr key={rowIndex} className="odd:bg-white even:bg-accent/[0.035]">
                {columns.map((column) => (
                  <td key={column} className="border-b border-slate/5 px-4 py-3 text-slate/80">
